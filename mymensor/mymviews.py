...
def save_tagboundingbox(request):
    if request.method == 'POST':
        ...
        posx = float(received_json_data('x'))
        posy = float(received_json_data('y'))
        width = float(received_json_data('width'))
        height = float(received_json_data('height'))
        tagnumber = int(received_json_data('tagnumber'))
        try:
