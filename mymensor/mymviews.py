...
def tagSetupFormView(request):
    currentvp = 1
    qtyvps = Vp.objects.filter(vpIsActive=True).filter(asset__assetOwner=request.user).count()
    listoftags = Tag.objects.filter(tagIsActive=True).filter(vp__asset__assetOwner=request.user).filter(vp__vpNumber=currentvp).values_list('tagNumber', flat=True)
    qtytags = listoftags.count()

    if request.method == 'POST':
        currentvp = request.POST.get('currentvp', 1)
        currenttag = request.POST.get('currenttag',1)
        qtytags = request.POST.get('qtytags', qtytags)
        if qtytags==0:
            qtytags=1
        tag = Tag()
        try:
